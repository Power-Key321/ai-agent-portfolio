---
name: agent_harness
description: 乐高式 Agent 编排框架 — 把编排系统拆成六层 24 个可插拔模块，按任务类型自动装配最优模块链，螺旋收敛逼近用户真实意图。
description_zh: 乐高式 Agent 编排框架 — 预处理→拆解→调度→上下文→执行→交付 六层 24 个模块，6 类任务策略，三层意图路由，量化门检与退化检测，反馈自优化闭环。适用于多类任务混杂、需要统一编排层与失败自愈的场景；依赖 Python 3.10+ 运行环境。
description_en: Lego-Style Agent Orchestration Framework — decompose the orchestration system into 6 layers × 24 pluggable modules; auto-assemble the optimal module chain per task type; spiral convergence toward the user's true intent. 6 task strategies, 3-tier intent routing, quantitative gating with degradation detection, feedback-driven self-optimization loop.
display_name: 乐高式 Agent 编排框架
display_name_en: Lego-Style Agent Orchestration
